...
local dpi = require("beautiful.xresources").apply_dpi --used for curved borders and seperator in wibar

-- Theme handling library
local beautiful = require("beautiful")
-- Notification library
local naughty = require("naughty")
-- Let dunst handle notifications
...
-- Widget wrapper with rounded border
local function widget_box(widget)
	return wibox.widget({
		{
			{
				widget,
...
local separator = wibox.widget({
	markup = '<span foreground="#45475a"> | </span>',
	align = "center",
	valign = "center",
...
-- Fix key release events
local old_connect_signal = client.connect_signal
client.connect_signal = function(name, ...)
	if name == "key::release" then
		return
	end
	old_connect_signal(name, ...)
end
...
local function update_temp()
	awful.spawn.easy_async_with_shell(
		"sensors coretemp-isa-0000 | grep 'Package id 0' | awk '{print $4}'",
		function(stdout)
			local temp = stdout:gsub("\n", "")
			cpu_temp_widget:set_markup('<span font="JetBrainsMono Nerd Font 10"> ' .. temp .. "</span>")
		end
	)
end
...
local function show_power_menu()
	awful.spawn("bash /home/pratham/.config/awesome/powermenu.sh")
end

local power_button = wibox.widget({
	markup = '<span font="JetBrainsMono Nerd Font 12"> ⏻ </span>',
	align = "center",
	valign = "center",
	widget = wibox.widget.textbox,
})
...
awful.screen.connect_for_each_screen(function(s)
	...
	s.mywibox:setup({
		...
		{ -- Right widgets
			layout = wibox.layout.fixed.horizontal,
			widget_box(cpu_temp_widget),
			separator,
			widget_box(ram_widget()),
			separator,
			...
			separator,
			widget_box(wibox.widget.systray()),
			separator,
			widget_box(mytextclock),
			separator,
			widget_box(power_button),
			separator,
			widget_box(s.mylayoutbox),
		},
	})
end)
-- }}}
